...
// What to call a building on screen: her nickname if she set one, else the name it was imported with.
function bname(b) { return (b && (b.display_name || b.name)) || ''; }
function initial(s) { const m = String(s || '').match(/[A-Za-z]/); return m ? m[0].toUpperCase() : '#'; }

const PAYMENT_METHODS = ['Cash', 'Check', 'Zelle', 'Venmo', 'Bank transfer', 'Card', 'Online (bank)', 'Online (card)', 'RentRedi', 'Money order', 'Other'];
const UNIT_KINDS = ['apartment', 'garage', 'parking', 'storage', 'other'];

module.exports = { bname, initial, esc, money, money0, dateLong, dateShort, CATEGORIES, categorizeLine, categorizeReceipt, PAYMENT_METHODS, UNIT_KINDS };
