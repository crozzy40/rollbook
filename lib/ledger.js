...
function tenancyBalance(tenancyId) {
  ...
  const payments = db.prepare('SELECT id, date, amount, method, reference, memo, source FROM payments WHERE tenancy_id = ?').all(tenancyId);
...
function portfolioRows() {
  const rows = db.prepare(`
    SELECT t.id AS tenancy_id, t.tenant_name, t.rent, t.deposit, t.lease_start, t.lease_end, t.lease_text, t.phone, t.email, t.notify,
           u.id AS unit_id, u.label AS unit_label, u.kind AS unit_kind,
           b.id AS building_id, b.name AS building_name, b.display_name AS building_display, b.demo
  ...
  return rows.map(r => {
    ...
    return { ...r, building_name: r.building_display || r.building_name, balance, credit: alloc.credit, aging: ag, paidThisMonth, chargedThisMonth, lastPayment, status };
  });
}
...
// Unit board: every building in her order, every unit with its active tenancy row (or vacant).
function unitBoard() {
  const bl = db.prepare('SELECT id, name, display_name, photo, address, demo FROM buildings ORDER BY sort, name').all();
  const units = db.prepare('SELECT id AS unit_id, building_id, label AS unit_label, kind AS unit_kind, sort FROM units ORDER BY sort, label').all();
  const byUnit = {};
  for (const r of portfolioRows()) byUnit[r.unit_id] = r;
  const buildings = bl.map(b => ({ ...b, units: [] }));
  const idx = {}; buildings.forEach((b, i) => idx[b.id] = i);
