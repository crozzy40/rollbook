...
function layout(ctx, { title, active, body, wide = false, extraHead = '', script = '' }) {
  ...
  const navItems = [
    ...
    ['/messages', 'Reminders', 'messages'],
    ['/import', 'Import workbook', 'import'],
    ['/export', 'Export to Excel', 'export'],
    ['/settings', 'Settings', 'settings'],
  ];
  const nav = navItems.map(([href, label, key]) =>
    `<a href="${href}" class="${active === key ? 'on' : ''}">${label}${key === 'owed' && owed ? `<span class="count">${owed}</span>` : ''}${key === 'work' && ctx.newReports ? `<span class="count">${ctx.newReports}</span>` : ''}</a>`).join('');
  ...
  const tabs = [['/', 'Today', 'home'], ['/delinquency', 'Who owes', 'owed'], ['/units', 'Units', 'units'], ['/expenses', 'Expenses', 'expenses'], ['/more', 'More', 'more']]
    .map(([href, label, key]) => `<a href="${href}" class="${active === key || (key === 'more' && ['leases', 'work', 'reports', 'messages', 'import', 'export', 'settings'].includes(active)) ? 'on' : ''}">${label}${key === 'owed' && owed ? `<span class="count">${owed}</span>` : ''}</a>`).join('');
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
function morePage(ctx) {
  const items = [['/leases', 'Leases', 'End dates, soonest first'], ['/work', 'Work orders', 'Repairs open and done'], ['/reports', 'Reports', 'Year at a glance'], ['/messages', 'Reminders', 'Rent notices and your weekly summary'], ['/import', 'Import workbook', 'Bring in a building from Excel'], ['/export', 'Export to Excel', 'Workbooks, rent roll, backup'], ['/settings', 'Settings', 'Rent rules, password, demo data'], ['/logout', 'Sign out', '']];
    ...
    <div class="two-col">
      ...
      <section>
        ...
        ${leases.length || work.length ? `<ul class="timeline">
          ...
          ${work.slice(0, 5).map(w => `<li><span class="when">${dateShort(w.opened_at)}</span><span>${w.source === 'tenant' ? 'Reported: ' : 'Open: '}<a href="/work/${w.id}/edit">${esc(w.title)}</a> <span class="muted">${esc(w.building_name)}${w.unit_label ? ' ' + esc(w.unit_label) : ''}</span></span><span class="muted"></span></li>`).join('')}
  ...
  return layout(ctx, { title: isNew ? 'Move in' : 'Edit tenant', active: 'units', body: `
    ...
    <form class="form" method="post" action="${isNew ? `/units/${unit.id}/move-in` : `/tenancies/${tenancy.id}/edit`}">${csrfField(ctx)}
      <div class="fields">
        ...
        <div class="field"><label for="nt">Reminders</label><select id="nt" name="notify">${[['both', 'Email and text'], ['email', 'Email only'], ['sms', 'Text only'], ['off', 'Do not contact']].map(([v, l]) => `<option value="${v}" ${(t.notify || 'both') === v ? 'selected' : ''}>${l}</option>`).join('')}</select><div class="help">Used for rent notices. Their pay link keeps working either way.</div></div>
